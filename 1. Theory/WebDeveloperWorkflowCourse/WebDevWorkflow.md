...
- `observe the general flow`(just a preview example )
  ...
  git checkout -- . (to recover files from repo)
...
- In the next lesson we write a bit of JavaScript code together. Please note that I move uncharacteristically fast and don't explain the code in 100% detail like I prefer to.
  ...
  Rest assured that later in the course when we actually start writing JavaScript code to power the front-end of our website I will indeed take the time to explain our code in full detail and cover the "why" behind what we're doing
...
- inside App.js, `create a test alert`:
...
- we want webpack and postCSS to see this `import` line and actually `replace` this line with the contents of this `_large-hero.css` file
...
- `Let's make sure that our CSS is still working without a full reload though`.
...
- The one final thing that I want to do is `add high DPI copies for each cropping`.
  ...
  <source
    srcset="
      assets/images/our-start-portrait.jpg        382w,
      assets/images/our-start-portrait-hi-dpi.jpg 764w
    "
  ...
  <img
    srcset="
      assets/images/our-start-landscape.jpg         800w,
      assets/images/our-start-landscape-hi-dpi.jpg 1600w
    "
...
- Let's move on and make this image responsive. So back in our html we look up just a few lines. We see that image here
  ...
  <div class="wrapper wrapper--medium wrapper--b-margin">
    <img
      srcset="
        assets/images/first-trip-low-res.jpg 565w,
        assets/images/first-trip.jpg,
        assets/images/first-trip.jpg
      "
      alt="Couple walking down a street."
  ...
  <div class="wrapper wrapper--medium wrapper--b-margin">
    <img
      srcset="
        assets/images/first-trip-low-res.jpg 565w,
        assets/images/first-trip.jpg         976w,
        assets/images/first-trip.jpg
      "
      alt="Couple walking down a street."
    />
  ...
  <div class="wrapper wrapper--medium wrapper--b-margin">
    <img
      srcset="
        assets/images/first-trip-low-res.jpg  565w,
        assets/images/first-trip.jpg          976w,
        assets/images/first-trip-hi-dpi.jpg  1952w
      "
      alt="Couple walking down a street."
    />
  </div>
...
28. _Attention to Detail (Part 2)_
...
- Next let's focus on fine tuning the `typographic for these paragraphs`.
  ...
  <p>
    ...
    <strong>reprehenderit in</strong>
    voluptate.. aliqua.
  </p>
...
- let's see what things look like on a small screen. the first thing that I see is that we should probably `add a bit of vertical margin` for small screens `between this image and this heading.`
  ...
  ```html
  <div class="row__medium-4 row__medium-4--larger row__b-margin-until-medium"></div>
  ```
...
30. _Styling Our Features (Part 1)_
...
- However we need `a bit of margin` or gutter between the two columns.
  ...
  <div class="row row--gutters">
    ``` - But if we look at the design file I think that this our features section `calls for an
    extra wide gutter`. - So why don't we just go ahead and `create a new modifier class of
    row--gutters-large` ```css
    <div class="row row--gutters-large"></div>
  </div>
  ````
...
- Why don't we see how things look on a smaller screen `I see several things that I would like to adjust for mobile.`
  ...
  - So why don't we say within our baseline Stiles (&\_\_icon) `margin-bottom:` go is just a little over half of a ramp so `.6rem`
...
- Let's begin `styling the actual content of the testimonial.`
  ...
  - let's `create a new rule that targets that class` &\_\_photo {}
...
- The next thing that I notice is that `each of our testimonials is a different height`. But if we look at the `design file they are all equal height.`
  ...
  ```html
  <div
    class="row row--gutters row--equal-height-at-medium row--gutters-small row--t-padding generic-content-container"
  >
  ...
  - And `right before the final closing curly bracket` let's create a new CSS comment _/ begin equal height rules_/We are including this just to stay organized.
...
- Now let's see how things look `on smaller screens`.
  ...
  ```html
  <div class="wrapper wrapper--no-padding-until-medium"></div>
  ```
  ...
  - let's give it a modifier class of `testimonial--last`.
    ...
    <div class="row__medium-4">
      <div class="testimonial testimonial--last">
        <div class="testimonial__photo"></div>
      </div>
    </div>
    ```
  ...
  - `The overall testimonials div` has a class of page section. Why don't we go ahead and add on another modifier `page-section--no-b-padding-until-medium`.
    ...
    <div
      id="testimonials"
      class="page-section page-section--no-b-padding-until-medium page-section--testimonials"
    ></div>
    ```
...
- and jump back to our html The next instance of medium is on `the wrapper div`
  ...
  - Let's `change that to large`
    ...
    ```html
    <div class="wrapper wrapper--no-padding-until-large"></div>
    ```
...
- let's get started on `adding the high DPI copies` (for the images).
  ...
  <img
    sizes="160px"
    srcset="assets/images/testimonial-jane.jpg 160w, assets/images/testimonial-jane-hi-dpi.jpg 320w"
    alt="Jane Doe"
  />
  ```
...
- Now before we save this `we can make sure that things are set up correctly` by `adding a -i` at the end of both of these.
  ...
  <img
    sizes="160px"
    srcset="
      assets/images/testimonial-cat-i.jpg        160w,
      assets/images/testimonial-cat-hi-dpi-i.jpg 320w
    "
    alt="Cat McKitty"
  />
  ```
...
37. Hexadecimal to RGBA Color In The Next Lesson
...
- if we check out the browser we want to go ahead and `float this content to the right` so that it's positioned here.
  ...
  ```html
  <nav class="primary-nav primary-nav--pull-right"></nav>
  ```
  ...
  - and then we want to `tell the list items to no longer have a bullet.`
    ...
    - Now if I was going to follow the Bem philosophy 100 percent I would give a class to this element ,(ul), named primary-nav\_\_list

    - and I would give each list item element a class of primary-nav\_\_item

    - and I would even give each link a class like primary-nav\_\_ink.
...
- So my first thought is that `for small screens we probably don't need to show the navigation links and get in touch button right away` - for small screens we can probably hide this content until the user taps a menu icon.
...
40. CSS Grid (Overlapping Content)

    Hello, everyone, this is a quick note from Future Brad, so I'm filming this video in the beginning

    of twenty twenty one, and I know that in the previous video I said that we were going to jump in to

    a new chapter about JavaScript and we will very shortly.

    But first, while we're still focused on CSI and the graphic design of our site, I do want to show

    you a more modern way of handling the positioning and centering and sort of overlapping of this content.

    So when we originally set this up together, we used absolute positioning to handle both the overlapping

    nature, but also the positioning of this text content.

    Now, there's nothing wrong with using absolute positioning.

    However, in 2021 and beyond, we do now have a more modern tool at our disposal.

    And since this course is all about showing you the modern toolset, I want to be sure that we cover

    it here together.

    So what we're going to do in this video is update this large hero area so that it uses something called

    CISSP grid instead of absolute positioning.

    If you're not super interested in CSC and you're more interested in moving on to JavaScript, by all

    means, you can skip this lesson.

    But if you enjoy learning about success, then I think this video will be a great exercise anyway.

    So let's get started.

    So if you've never heard of CSI Grid before, that's OK.

    So I have this other tab with code pen open and here we can answer the question, first of all, of

    what in the world is sixths grid?

    Now, we're not looking at six grid right now yet.

    This is just sort of an empty skeleton.

    So in the HTML, we have an overall parent div and then inside of it we have to divs.

    Right.

    They have a class A and B, that's all we have.

    For the HTML example, let's look at the CSFs so that parent div it's a square, right.

    It has a width of 200 pixel height of 200 pixels and it has this border.

    The only other thing I already have here is I've given the A division, the B div halfway transparent

    red and blue backgrounds.

    OK, now let's actually get to access grid.

    So imagine we want to divide up this parent div maybe into thirds both horizontally.

    Right, so there would be one third another another, but also into thirds vertically.

    So like there's one, there's one and there's one.

    You'll see in a minute that the possibilities are endless.

    I'm just choosing that rule of thirds to begin.

    But basically on the parent div, all we would need to do is a display grid and then let's tell it how

    we want to divide up our space horizontally or in terms of columns.

    So we say grid template columns and if we want three columns, we could say 33 percent, 33 percent

    and 34 percent.

    Now if you're thinking I wish this could add up to a hundred percent would still be perfectly equal

    instead of this one being thirty four.

    Don't worry.

    We'll see a solution to this issue in just a moment.

    OK, now let's also tell it how to handle things vertically.

    So grid template rows say thirty three.

    Thirty three.

    Thirty four percent.

    OK, so you can almost imagine that there are these invisible lines in our parent Divx that are dividing

    up the content right into these third's.

    And we can see this by the size of our red and blue divs.

    However, what if we wanted the red divide to take up two areas instead of just one?

    Well, on this ative, we could just say grid column, give it a value of spane two.

    So now it spans the width of two grid columns.

    If we told it to spend three, it's going to do that.

    And then obviously the blue divide just jumps down to the next row.

    And at the same thing we could on the blue div, we could say grid column span to.

    Now, we can also do the same thing vertically or with rows instead of just horizontally with columns.

    So, for example, what if we wanted the blue div to take up or span two rows, you could just say on

    this Divx grid row span to.

    So we can almost think of this as like a game of Tetris, we're just fitting different sized blocks

    into a grid.

    However, unlike Tetris, we can actually have our blocks or divs overlap each other.

    However, before we get to that, I do want to show you how easy and flexible this is to work with.

    So, for example, instead of breaking this up into thirds, if you want it to have a fourth column

    up on our parent div, you could just include a fourth value.

    So, for example, if we take away 10 percent from this final values to say twenty four percent, then

    we could have a fourth column of ten.

    Right.

    So now the overall div is broken up into four columns.

    The fourth one is not very wide, but you get the idea so you can have as many different columns as

    you want.

    You just include more space separated values here.

    And you always just wanted to add up to the full available width so I wouldn't go over one hundred percent.

    So this is very flexible.

    You're not limited to just three or four columns, but let's move on.

    So let me put this back to thirty four percent.

    Now, remember earlier I said we would address this issue of how this doesn't add up perfectly to one

    hundred percent or it does, but they're not perfectly equal.

    Right.

    Instead, you want it to be maybe thirty three point three three three three just infinitely point three

    three three.

    Well, to get around this, instead of actually using numbers, we can let the browser calculate that

    math.

    So instead we can say one A4, one ethar, one far, far stands for a fraction unit.

    And now we still have three columns, but the browser will perform that math too evenly distributed

    amongst them.

    So if you wanted for equal width columns, you could just have another one, F.R..

    All right, if you wanted five columns, you get the idea, OK, you can use these with the rose as

    well.

    Now, what I want to show you is back to the actual task at hand, which is how would you handle overlapping

    content?

    Well, this is why I made the device halfway transparent so you can see when they're overlapping.

    So, for example, let's try to move this blue div up so that it's sitting right here instead.

    So if we go down to our blue div selecter, instead of just giving grid column of value of how big we

    want it to be, you know, saying span across two columns, we can instead give it to values and say

    it should start in the first column slash.

    We want it to take up two columns.

    So the first value is the placement in the grid and then this is the size.

    So we can do the same thing with Row.

    Say we want it to begin in the first row and we want it to take up to OK.

    Now that did change it, but we're still not getting the overlapping feature that we want.

    So we just need to specifically tell the red div where we want it to be placed as well, and then the

    grid will let us have the overlapping feature.

    So on this red div for the grid column value, we can just say start in the first column, span all

    the way across and then for grid row again, start in the first one and we really just need it to spend

    one.

    Cool, so now you can see they're both taking up that same space or they're overlapping, so what this

    means is that sixth grade by default will try to align your content in a way that doesn't overlap.

    Right.

    It's going to try to populate the grid in a way that makes sense where the content fits together like

    Tetris blocks.

    However, if we specifically say where they should be placed and not just their size, then we can create

    this overlapping feature.

    Now, just to make sure that you understand how this first value works, this placement value instead

    of the size value.

    Let's imagine we wanted the blue div to be in this top right corner like this.

    So on that blue div four grid column, this starting value of where you want it to be placed instead

    of one, we can just say two.

    So that's saying we want it to begin at the start of the second column.

    You can do the same thing with RO.

    We could say we want it to start at the beginning of the second row.

    I think you get the idea.

    But obviously fine tuning those values allows us to create this overlapping effect.

    And on a random note, if you're wondering why there's this tiny one pixel white gap, that's just because

    I'm zoomed out a little bit.

    If I zoom in to the natural one hundred percent view, you can see that goes away.

    Cool.

    So now that we have a general idea of what CSFs Grid is and how to use it, let's jump back to the task

    at hand to remember, instead of using absolute positioning to place this text content here.

    So we're centering it both horizontally and vertically within this container.

    And also, instead of using absolute positioning so that this overlaps or sits on top of the responsive

    picture or image, we're going to use access grid to power, all of that.

    So let's get started in our text editor.

    I have our large heterodoxy file open and let's begin by removing this position relative from the overall

    block.

    OK, and then in the text content area, we don't need position absolute and actually we don't need

    any of these position related properties like Top Transform left with someone to delete all of that.

    So now all I have left there is the padding top and the text line.

    OK, now if we save that, we're going to lose all of our positioning, right?

    So now it looks like this instead.

    Now let's use siestas grid.

    And actually, before we get into that, I do want to make a tiny change to our HTML.

    Now, if you want to leave your absolute positioning in place, you don't need to make this change to

    the HTML.

    But if you're using CSC grid like me, here's what we're going to do in our HTML.

    Find the large hero area.

    And what I'm focusing on is this picture element.

    So right now, the image element inside of the picture element that has a class right at the end of

    this line, we see class equals large hero image.

    So what I'm going to do is just cut this class, so get rid of it here and then place it on the overall

    picture element like this.

    So go ahead and save that this way, what we're going to do is we're going to let a large hero be the

    sort of parent element that has the grid template.

    And then these are the two children, right?

    The overall picture element and our large hero text content.

    So now let's jump back into the success on our overall parent block.

    We can just say display grid and then also grid template columns, lists.

    Honestly, just have one column.

    We only need one column because we're not trying to push any of the content to the left or the right

    or the top or the bottom.

    We're really just trying to get them to overlap each other.

    OK, now let's adjust this selector, because this is now no longer pointing towards the image element

    itself.

    This is pointing towards that picture element.

    So inside here, I would just cut this property to live inside an image selector like that.

    And then for this actual overall underscore, underscore image class, this is where we can place it

    on the grid so we can say grid column.

    Let's just have it begin in the first column.

    OK, and then let's do the same thing for our text content.

    Let's just say grid column.

    Let's place it in the first column.

    Give that to save.

    Woops, so they're still not overlapping, we do need to define the replacement as well.

    So on the image, we can just say grid row, place it on the first do the same thing for text content,
    ...
    Now, the text content almost already looks like it's centered it looks centered horizontally because

    we have text the line center, but it's definitely not centered vertically.

    To fix that, we can just go into that text content block and just give it another property of a line

    self.

    This is a sixth grid related property and you can just say center.

    Let's give that a save.

    Cool.

    So overall, we have the same exact layout and it's still responsive, only now we had a great opportunity

    to practice siestas grid.

    Now in our very next video, we're going to continue practicing CSFs Grid.

    So when it comes to creating our column layouts, right, there's our again to column layout.

    And then down here we have a three column layout.

    Well, in the next video, instead of using float's or flex box, we're going to learn how to use CSC

    Grid for our column layout setup.

    OK, and then after that, I promise we will move on to the JavaScript chapter.

    Let's keep things rolling and I'll see in the next video.

41. CSS Grid For Columns

    Hello, everyone, in this video, we're going to continue to learn about sixth grade, so in our previous

    video, we learned how to use sixth grid in place of absolute positioning.

    Now, in this video, we're going to learn how to use it in place of floats.

    So there's one column.

    There's a second column.

    Again, here's a two column layout.

    Write one column, two column.

    And the down here we have obviously a three column layout.

    So we're going to learn how Sixth Grid is the perfect fit for our theoretical 12 column grid that we

    set up earlier in the course.

    Let's go ahead and get started.

    So let's jump into our text editor and the CFS file that we're interested in is in app assets styles,

    and then it's in the modules folder called ROE or underscore road access.

    OK, so big picture.

    Our goal in this video is to no longer use float left to create multiple columns.

    So if we're not going to rely on float, we don't need this clear fix so we can get rid of that.

    We can keep this top padding modifier class.

    That's OK.

    However, after that we begin to see classes about gutters.

    Right.

    The space in between columns.

    Now, for the next 10 seconds, we can ignore these first three gutter rules and instead pay attention

    to these three where the selector is looking for any direct child that is a div.

    So these three rules that I've selected, this is what's currently adding the gutter or spacing in between

    our columns.

    Now, this was a great way of handling it when we were using floats.

    But now if we're going to use excess grid, we don't need to tell each child or each column to create

    that spacing.

    We can instead just tell the one parent or the one row container div to set up that gutter in between

    columns.

    So for these three rules that are looking for a direct div child of these classes, we can actually

    just completely delete these three rules.

    OK, and then the three rules that were right above that where we got a bit creative and used a negative

    margin for the overall row.

    Well, now these are the selectors for the overall parent element.

    So we can actually let siestas grid add the space in between the columns for us.

    So check this out instead of margin.

    Right.

    You can just set this to column gap and then we don't need a negative value any longer.

    Right?

    We're not using that creative negative margin fix.

    So overall, we're just saying let there be sixty five pixels in between each column and then let's

    do the same thing here too, so you can just copy and paste the property name, do that for this rule

    as well.

    Column gap, make it a positive value instead negative and then finally do it one more time here in

    positive value.

    Cool.

    Now really quick to show you how column gap works and also why perhaps a pixel value here is not always

    the best choice.

    I do want to jump into a code pen demo.

    OK, so for the HTML in this quick demo there's a parent div and then inside it there are 12 divs,

    which is nothing but a number inside them.

    And then the next we're saying display grid grid template columns.

    Now remember earlier in the course we set up a 12 column grid just because 12 is easily divisible by

    several useful numbers.

    So the way that this is working, remember, you could say one F.R., one F.R. for two columns and have

    a third for three.

    Have a fourth four for.

    The point here is you wouldn't want to have to spell this out 12 times, right, that line of code would

    get very long.

    So you can instead use this repeat tool.

    And then first you say how many times you want to repeat something and then comma, the second value

    is what you want to repeat.

    So this is just the equivalent of actually saying one F.R., one far right, 12 times.

    So it's a bit cleaner this way.

    But then we have Collum gap.

    So at the moment it's set to 10 pixels.

    But really quick, I want to show you how you can visualize the way that your grid is set up.

    So if you use your browser dev tools.
    ...
    Click over here, choose, inspect and then if I choose the parent div that has display grid set on

    it here you can see that column gap property.

    Now check this out.

    If I click on to that, I can then use my arrow keys to increase or decrease it.

    However, if I don't click but just hover my mouse on top of that parent div here right over in the

    left hand mark-up view as I hover over that div notice what's going on in the preview area.

    Essentially Google Chrome shows the columns themselves as blue and it shows the column gaps as purple.

    Now, what I want to show you here is if I increase the column gap to 11 pixels, 12, 13, 15, you

    can see the purple column gaps the space in between the columns.

    That's becoming wider.

    However, because we have a 12 column grid, there needs to be space for 12 of those gutters or gaps.

    Right.

    And if I increase this pixel value past a certain threshold, you'll notice that it starts to take up

    really more space than is available on the screen.

    Notice right there, it's now falling off the right edge of my browser window.

    It's just a simple issue of there's not enough horizontal space available to fit that many gaps of this

    particular size.

    Now, in a world of responsive web design where you and I never truly know the exact size of a browser

    window, I think it makes sense if you're going to have rather large gaps to instead of using pixels,

    maybe use percentages.

    Now, yes, if you're going to have small gaps, you can use pixels.

    That's a safe bet.

    But in order to avoid the situation where the gaps are taking up more space than is even available,

    I say let's go with percentages.

    So all of that was just to let you know that back in our code, here's what I'm going to do for the

    base gutters rule instead of sixty five pixels.

    Why don't we set this to maybe five point three percent for the large modifier?

    Why don't we set it to eight percent and then for the small modifier, let's go with three point six

    percent.

    Now before we move on, I do want to let you know you don't always need to use percentage values for

    column gaps like this.

    We really only need to do this because we have 12 columns.

    Right.

    So, for example, back in code again, if I set this to only be, you know, a three column grid,

    well, then we could get away with relatively large pixel values for the gaps.

    And even on a very small smartphone, that would be a safe bet.

    Right.

    That's not going to overfill things horizontally.

    So you really only need to be aware of the situation where you're using more horizontal space than is

    available if you're having a large number of columns.

    Right, like 10, 12 beyond that.

    Right.

    Some sort of overall total pixel value that's usually going to be larger than three to four to five

    hundred pixels, if that's the typical size of maybe a smartphone.

    Anyways, that's enough on gaps.

    Let's go ahead and move on.

    Why don't we do what we perhaps could have done first, and that is tell the overall parent row that

    it should use display grid.

    Now, I'm not going to add that right here because we're taking a mobile first approach and we probably

    don't want any columns whatsoever.

    Right.

    We just want a single column view for tiny mobile screens.

    So what I'm going to do is instead look for our first media query mix in right about here, right where

    we're saying at medium and inside there is where I will say that the overall row parent should use display

    grid.

    OK, let's also say that it should be a 12 column grid.

    So grid template columns.

    Now, again, this is where you could say one F.R., one FLIR, one F.R., four, three columns, two,

    four, two.

    But if you want 12, instead of spelling it out 12 times, you can just say repeat inside these parentheses.

    How many times do we want to repeat something?
    ...
    OK, now while we're at it, let's also move our gutter rules to live inside this at medium media query

    as well.

    So I'm just going to take these three gutter rules, maybe cut them.

    And then down here right below those two lines we just wrote right with the repeat 12, if I just paste

    that in.

    OK, at this point, we can start scrolling down and removing anywhere we see float left or a specific

    width property.

    So, for example, I see this rule of medium dash for well, we're no longer using floats.

    We're using 6th grade.

    So you can delete that float left line and then we don't need to give it a width value so we can delete

    width instead.

    In sixth grade we could just say grid column span four.

    So the math is very easy, right.

    In this concept of a 12.

    Oops, let me add a semicolon there.

    In this concept of a 12 column grid, we don't even need to do the math anymore of.

    Right.

    33 percent or six out of 12 is 50 percent.

    Now, we can actually just stay within that concept of 12 columns in the grid.

    Right.

    For just means that you should span across the width of four columns.

    I'm actually going to remove this dash dash larger modifier column, if you want to call them.

    That's wider than four.

    Just said it to five.

    Right.

    So I'm going to get rid of that sort of in-between class.

    OK, let's do the same thing for this medium six and medium eight so you can actually just copy this

    line to avoid typing and get rid of the float in the width and just change this to be six.

    Right.

    Medium six should span six.

    Do the same thing for eight.

    So delete that two lines, pasted it and change it to eight.

    I'm going to get rid of this dash dash, smaller modifier rule.

    OK, let's keep going down now.

    We're in the at large media query.

    So again I'm just looking for these rules where we no longer need to float.
    ...
    Yes, I'm going to get rid of the larger modifier class.

    Large six, again, delete, float and width, change this to six, do the same thing for eight, change

    that eight and also get rid of this dash dash smaller modifier class.

    Cool.

    Let's go ahead and save and see what we have so far to back up at the top.

    Cool.

    This is a two column area that's working just fine.

    Scroll down a bit again to column layout looks good.

    However, if we scroll down to the bottom testimonials area, you can see that the middle column is

    a different size, right?

    These three columns are not equal width.

    And this is because in order to achieve this equal height design, we used flex box instead of float's

    and flex box pays attention to how much content an item has.

    So because there's less text here than in this column, it gives it less space.

    So if we want our three equal width columns here, we just need to remove flex box or display flex from

    this modified row called equal height at large.

    So in our case, if we scroll down a bit and here we see equal height at medium, I can't remember exactly

    where we're using that in our HTML.

    I'm not sure if we're using it at all, but I do know we are using this equal height at large for that

    bottom testimonials area.

    So if you just get rid of this display flex here and give that save notice, that fixes the testimonials.

    And now they are three equal width columns.

    So big picture, we don't want display flex on the overall row.

    Right.

    That one single div that contains the other three.

    However, we do want to tell each of these columns to use display flex so that they still have equal

    height.

    So that's what this rule down here is doing.

    However, we don't need float none.

    We just want display flex.

    And actually we would only need to set that once you're in this media query.

    So I'd probably just take this entire rule and just cut it to live inside this media query mix in here.

    Give that a save.

    Everything still looks great.

    OK, next, let's make sure that our layout stays intact on smaller screens.

    So let me scroll back up and let me start making my screen a bit more narrow.

    Cool.

    So at that medium break point, right, we still have to call.

    But once the screen gets down to this size, it snaps to the single column layout.

    Perfect.

    Let's test out the our features area.

    So on a small screen, we have single columns.

    Once we get to about that size, cool, we get the two column layout perfect.

    But let's try the testimonials area.

    So it looks good on a larger screen.

    If I start shrinking the screen looks ok.

    It looks OK.

    That looks very awkward.

    And then on a tiny screen we do successfully jump down to a single column.

    So we really just need to fix this in-between area.

    That does not look right.

    And I can tell you exactly what's going on here.

    This is our at Medium Media Query Mixin.

    So when it snaps to here, this is still our large media query.

    This is our app medium.

    And then once this is tiny like this, this just isn't any of our media query mixes.

    Right.

    This is the mobile first tiny view.

    So it really the problem is the medium and I can show you exactly what's going on.

    So back in our code, back up towards the top.

    When we have our first at Mixin at Med., we said that we should use a display grid and you should have

    a 12 column grid, so this is going to be applied for any screen that is medium or larger, meaning

    this is still going to be applied to the large media query breakpoint when in reality, once the screen

    gets to be about this size right here, I think we should just collapse this down to a single column

    view.

    So here's what we can do to fix that inside at medium media query right below these gutter rules, but

    before any of the actual medium for medium six.

    So right above maybe medium for, let's say, ampersand, underscore, underscore, large dash and then

    let's just list all of them.

    So let's go all the way from one to 12.

    So I'm just going to copy and paste this really quick.

    I won't bore you with this.

    You can follow along if you want to, but you don't need to.

    OK, so I just created 12 rules for all of these potential large class names and then I'm just going

    to curly brackets for the actual body.

    I would just say grid dash column should span 12, right.

    The entire full width.

    Now, the ordering here is important in the sense that I want these to come before the medium class

    names.

    This way, if you were creating a layout where you wanted for Divs to fit on a row for larger screens,

    but then you wanted only three of them to be able to fit on a medium screen, you could mix and match

    these two different size class names the way that perhaps you've seen it being used in something like

    bootstrap.

    Let's go ahead and give this a save and cool.

    So now the testimonials area, there is a three column and then as I shrink down, it just immediately

    goes to the single column layout.

    Perfect.

    So the task at hand is complete, we've now set up our column layouts using CSFs Grid instead of float's.

    However, before we bring this video to a close, I do want to let you know that while the success that

    we used in this video works, this is not the most graceful way possible of handling responsive design

    when it comes to success grid.

    Quite the opposite, really.

    This video is just me trying to shoehorn CCWs grid into our existing HTML structure and CSS class naming

    scheme.

    So if, for example, you were working on a brand new, fresh project where you didn't already set up

    a 12 column grid, I probably wouldn't even take this approach.

    Now that we have seized grid in our toolbox.

    What I mean by that is this way of organizing things made sense when we were using floats, because

    with floats, you have to get creative with the math and the width values and the gutters and the negative

    margins.

    There's just a bit of complexity to it.

    So it made sense that we would want to extrapolate all of that away into a reusable, albeit rigid class

    structure like this.

    However, if floats are out of the picture, see us as grid is so powerful and so flexible and so easy

    to work with, that if I were to rebuild this site from scratch, I don't think I'd even worry about

    having a singular row component that powers all of our different column layouts.

    I'd probably just treat each section of the site as its own independent area and write unique, says

    grid code for each section, big picture, every site and every project and every development team is

    different.

    So there isn't one approach.

    That's right for every situation.

    But I do encourage you to learn more about CSFs Grid if it interests you at all.

    In fact, I've created this YouTube video.

    I'll leave a link to this in the description of this lesson.

    But at around the 36 minute mark, I start talking about sixth grade and in particular at about the

    44 minute mark.

    I begin to show you how you don't even need media queries in order to have different column layouts

    for different screen sizes when you're using CSFs Grid.

    Yes, that's right.

    CSFs Grid can handle that on its own without even needing media queries whatsoever.

    Now, again, there's no right or wrong way to set up your site's design.

    But if you're interested, I think it's something worth looking into.

    OK, that is going to bring this video to a close.

    In our very next lesson, we're going to jump in to the JavaScript chapter.
...
43. The JS Module Pattern and "webpack"
...
- Now in terms of organization wouldn't it be nice if the blueprint or this constructor function or this class could `be tucked away into its own separate file.`
  ...
  - anytime we create a new feature using JavaScript`each new feature should have its own new separate file in this modules folder`.
...
44. How to Avoid Spaghetti Code
...
&__logo {
  ...
  top: 0;
  left: 50%;
  transform: translateX(-50%) scale(0.8); /// add
...
- now at the moment `I don't believe there are any elements in our html file with this class` (`open-modal`) so let's fix that right now.
  ...
  ```html
  <p>
    <a href="#" class="btn btn--orange btn--large open-modal">Get Started Today</a>
  </p>
  ```
...
- Let's `keep the local copy of our repo clean` by deleting unnecessary branches.
  ...
  - So let's go ahead and `delete that locally`. - `git branch -d count-to-15`

  - Let's also `delete the count-to-10` branch - `git branch -d count-to-10`
...
- And now we just need to `jump into our html file to tell lazy sizes which images we want to be lazy loaded` as opposed to just loading the standard or typical way.
  ...
  - currently this image doesn't have any classes. So let's give it an attribute class and let's give it a value of lazy load. `class="lazyload"`
    ...
    <picture>
      <source
        sizes="404px"
        data-srcset="assets/images/our-start.jpg 404w, assets/images/our-start-hi-dpi.jpg 808w"
        media="(min-width: 1020px)"
      />
      <source
        sizes="320px"
        data-srcset="assets/images/our-start-portrait.jpg 382w, assets/images/our-start-portrait-hi-dpi.jpg 764w"
        media="(min-width: 800px)"
      />
      <img
        class="lazyload"
        data-srcset="assets/images/our-start-landscape.jpg 800w, assets/images/our-start-landscape-hi-dpi.jpg 1600w"
        alt="Our founder, Jane Doe"
      />
    </picture>
    ```
...
- `let's apply lazy loading to the three images down in the testimonials section.`
  ...
  <div class="testimonial__photo">
    <img
      class="lazyload"
      sizes="160px"
      data-srcset="assets/images/testimonial-jane.jpg 160w, assets/images/testimonial-jane-hi-dpi.jpg 320w"
      alt="Jane Doe"
    />
  </div>
  ```
  ...
  <div class="testimonial__photo">
    <img
      class="lazyload"
      sizes="160px"
      data-srcset="assets/images/testimonial-john.jpg 160w, assets/images/testimonial-john-hi-dpi.jpg 320w"
      alt="John Smith"
    />
  </div>
  ```
  ...
  <div class="testimonial__photo">
    <img
      class="lazyload"
      sizes="160px"
      data-srcset="assets/images/testimonial-cat.jpg 160w, assets/images/testimonial-cat-hi-dpi.jpg 320w"
      alt="Cat McKitty"
    />
  </div>
  ```
...
- Let's `start with a piece of configuration that's the same for both dev` and build. And that is `our entry point`
  ...
  devServer: {
  before: function(app, server) {
  server.\_watch('./app/\*_/_.html')
  },
  contentBase: path.join(**dirname, 'app'),
  hot: true,
  port: 3000,
  host: '0.0.0.0'
  },
...
- Now we could use the web browsers fetch functionality to send off an asynchronous request but `I actually like to use a third party package named axioms`.
  ...
  constructor() {
  this.injectHTML()
  this.form = document.querySelector(".client-area**form")
  this.field = document.querySelector(".client-area**input")
  this.contentArea = document.querySelector(".client-area\_\_content-area")
  this.events()
  }
