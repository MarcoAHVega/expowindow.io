<!-- 
& responsive hero Block (part1).md
? (21.1) What does Mobile-first Mean
? (21.6) using media queries 
? (21.6.1) the large hero text is too large for a smartphone. 
? (21.6.2) Let's address the `top main title` first. 
? (21.6.3) we want to treat the mobile view as our baseline
? (21.7) install mixins 
? (21.8) creating  mixins for media queries
? (21.8.1) so now Let's try adjusting this subtitle. (`&__subtitle`)
? (21.8.2) let's try using all three mixins on this top `Your clarity` heading
...
? (21.6) using media queries 
   ...
   * I think it becomes immediately clear that 
   
? (21.6.1) the large hero text is too large for a smartphone. 
   
   So `let's adjust our CSS to fix that`.
...
? (21.6.2) Let's address the `top main title` first.
   ...
   * remember in the mobile first philosophy 
   
   ? (21.6.3) we want to treat the mobile view as our baseline` and modify things from there as necessary.
